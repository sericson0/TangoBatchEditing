...
def aufs_normalize(audio_segment: AudioSegment, target_lufs: float = -23.0) -> AudioSegment:
    """
    Normalize audio using AUFS (Average Unit Full Scale) / LUFS (Loudness Units Full Scale).
    
    Args:
        audio_segment: AudioSegment to normalize
        target_lufs: Target LUFS value (default -23.0, common broadcast standard)
...
def process_audio_file(input_path: Path, output_path: Path, target_lufs: float = -23.0) -> bool:
    """
    ...
    try:
        ...
        formats_requiring_ffmpeg = {'.flac', '.mp3', '.m4a', '.aac', '.ogg', '.wma', '.aiff'}
        ...
        # Ensure output path has the same extension as input
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  --target-lufs      : Target LUFS value for normalization (default: -23.0)")
        sys.exit(1)
